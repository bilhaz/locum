	...
	{   
        ...
        public function slide_save(){
            ...
            $data['link']=$this->input->post('link');
              ...
              $config['file_name'] = $filename.'_Pyzie';
            $config['allowed_types'] = 'gif|jpg|png|JPG';
            // initialize before upload
             $this->upload->initialize($config);
...
//            $data['products']=$this->admin_model->get_products_by_search_admin($key);  
            ...
            $data['products']=$this->admin_model->get_products_by_search_admin($key,30,$this->uri->segment(4));
            $config['base_url']=site_url("admin/product/$key/");
...
//            $config['use_page_numbers'] = TRUE;
        ...
        public function products($cat=0,$subCat=0)
	{   
            $data['breadcrumb']=' Products';
            // Pagination
            $this->load->library('pagination');
            $data['products']=$this->admin_model->get_products_by_cat_subcat($cat,$subCat,30,$this->uri->segment(5));
            $config['base_url']=site_url("admin/products/$cat/$subCat/");
            $config['total_rows']=$this->admin_model->get_products_by_cat_subcat_rows($cat,$subCat);
            $config['per_page']=30;
...
//            $data['authors']=$this->admin_model->get_authors();
        ...
        public function product_edit($id){ 
            ...
            $data['price']=$this->input->post('price');
//            $data['description']=$this->input->post('description');
            $data['subCatId']=$this->input->post('cat');
//            $data['specs']=$this->input->post('specs');
            $data['unit']=$this->input->post('unit');
...
//            $data['page_link_name']=$data['page_link_name'].".html";
	...
	{   
            echo "<script> var printTab=window.open('".site_url("admin/ordersDetailView/$id")."','_self') ;
	...
	{   
        ...
        function dbbackup()
        {
            ...
            write_file(site_url("backup/pyzie_DB_backup_$date.sql"), $backup); 
            $this->load->helper('download');
            force_download("backup/pyzie_DB_backup_".$date.'.sql', $backup);
